...
def register_worker(ip: str, data: dict):
    if ip in state.banned_ips:
        return WSMessage(
            WSMessageType.ERROR_RESPONSE, {"error": "Could not connect to worker"}
        )
    ...
    if data == None or (not "version" in data) or (not "max_concurrent" in data):
    ...
    if data["version"] > state.config["general"]["version"]:
        return WSMessage(
            ...
            {
                "error": f"Version mismatch, expected {state.config['general']['version']}"
            },
        )
    ...
    if data["discord_token"]:
        ...
        r = requests.post(
            ...
            headers=headers,
            auth=(
                state.secrets["discord"]["client_id"],
                state.secrets["discord"]["client_secret"],
            ),
        )
    ...
    with state.workers_lock:
        state.workers[worker_id] = Worker(
            worker_id, ip, auth_token.nonce, data["max_concurrent"], discord_id
        )
    if discord_id and not discord_id in state.current_leaderboard:
        with state.current_leaderboard_lock:
            state.current_leaderboard[discord_id] = state.LeaderboardObject(
                discord_id, discord_username, avatar_url, 0, 0
            )

    return WSMessage(
        WSMessageType.REGISTER_RESPONSE,
        {"worker_id": worker_id, "auth_token": auth_token.as_token()},
    )
...
def get_chunks(worker: Worker, data: dict):
    ...
    for file_id in state.sorted_downloadable_files:
            ...
            with state.chunks_lock:
                while current_size < file.get_total_size():
                    start = current_size
                        ...
                        current_size + file.get_chunk_size(), file.get_total_size()
                    )
                    chunk_id = str(uuid4())
        ...
        for chunk_id in file.get_chunks():
            ...
            if (
                state.chunks[chunk_id].has_worker(worker.get_id())
                and not state.chunks[chunk_id]
                .get_worker_status(worker.get_id())
                .get_complete()
            ):
        ...
        for chunk_id in file.get_chunks():
            ...
            if (
                state.chunks[chunk_id].get_worker_count()
            ...
            if (
                highest_chunk_id == None
                or state.chunks[chunk_id].get_worker_count()
                > state.chunks[highest_chunk_id].get_worker_count()
            ):
    ...
    for chunk_id in chunks_to_download:
        ...
        response[chunk_id] = {
            ...
            "url": file.get_url(),
            "range": [chunk.get_start(), chunk.get_end()],
        }
...
def upload_chunk(
    ...
    if state.chunks[chunk_id].get_worker_status(worker.get_id()).get_complete():
        return WSMessage(
            WSMessageType.ERROR_RESPONSE, {"error": "Chunk already complete"}
        )
    ...
    temp_storage_folder = os.path.join(
        state.config["paths"]["chunk_temp_path"], chunk_file_object.get_path()
    )
    # Check if a handle exits for this chunk
    chunk_path = get_chunk_instance_temp_path(
        chunk_file_object.get_id(), chunk_id, worker.get_id()
    )
    ...
    if file_handles[chunk_id].tell() != chunk.get_end() - chunk.get_start():
        return WSMessage(
            WSMessageType.OK_RESPONSE, {"ok": "Segment Received"}
        )  # Chunk not yet finished

    chunk.mark_worker_status_complete(
        worker.get_id(), chunk_hashes[chunk_id].hexdigest()
    ...
    with chunk.get_lock():
        ...
        if len(chunk_hash_set) > 1:  # There are mismatched hashes!
            ...
            for worker_id in list(chunk.get_workers()):
                ...
                state.assigned_chunks -= 1
                os.remove(
                    get_chunk_instance_temp_path(
                        chunk_file_object.get_id(), chunk_id, worker_id
                    )
                )  # Remove the chunk this worker downloaded
            return WSMessage(
                WSMessageType.OK_RESPONSE,
                {"result": "Upload had a mismatched hash, you can ignore this"},
            )  # We've processed the upload from the client, don't come back regardless of what happened

        # If the hashes weren't mismatched...
        if (
            chunk.get_worker_count() < state.config["general"]["trust_count"]
        ):  # Check that we have all the chunks responses we need
            return WSMessage(
                WSMessageType.OK_RESPONSE, {"ok": "Upload looks good so far"}
            )
        for worker_id in chunk.get_workers():  # Check that they're all complete
            ...
            if not worker_status.get_complete():
                return WSMessage(
                    WSMessageType.OK_RESPONSE, {"ok": "Upload looks good so far"}
                )  # If any of the workers aren't complete we just skip this
    ...
    with chunk_file_object.get_lock():
        destination_path = os.path.join(
            state.config["paths"]["storage_path"], chunk_file_object.get_path()
        )
        if os.path.exists(destination_path):
            return WSMessage(
                WSMessageType.OK_RESPONSE, {"ok": "Upload entire file complete!"}
            )
        ...
        if not file_complete:
            return WSMessage(
                WSMessageType.OK_RESPONSE, {"ok": "This chunk is validated"}
            )  # We're not yet done with the whole file despite being done with this chunk!
        ...
        state.sorted_downloadable_files.remove(
...
async def handler(websocket: ServerConnection):
    ...
    file_handles: dict[
        str, FileIO
    ] = {}  # File handles for each chunk this worker is uploading
    chunk_hashes: dict[str, object] = {}
    file_paths: dict[str, str] = {}  # Chunk to path
    while True:
        try:
            data = await asyncio.wait_for(
                websocket.recv(), timeout=state.config["general"]["worker_timeout"]
            )  # Timeout a worker after 10 minutes
            message: WSMessage = WSMessage.decode(data)
            response = WSMessage(
                WSMessageType.ERROR_RESPONSE, {"error": "Message failure!"}
            )
...
async def main():
    async with serve(
        handler, "", state.config["server"]["port"], max_queue=128
    ) as server:
